<template>
    <div id="feedback">
        ...
        <div class="button"><Button @click="feedback" :loading="loading" round block color="linear-gradient(45deg, #313635 0%, #888 100%)">提交反馈</Button></div>
    </div>
</template>
...
export default {
    ...
    methods: {
        feedback () {
            if (this.val === '') this.$toast('请输入反馈内容')
            else {
                this.loading = true
                setTimeout(() => {
                    this.$toast('已提交到客服人员，感谢您提供的反馈')
                    this.loading = false
                }, 1500)
            }
        }
